        ...
        match &next_move.move_type {
            ...
            MoveType::Resign => {
                match current_player {
                    Player::Player1 => {
                        GameState{ player_1: PlayerState{resigned: true, ..self.player_1.clone()}, moves, ..self.clone()}
                    },
                    Player::Player2 => {
                        GameState{ player_2: PlayerState{resigned: true, ..self.player_2.clone()}, moves, ..self.clone()}
                    }
                }
            }
        }
